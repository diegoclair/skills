# install.ps1 — bootstrap stub for the jira-tickets skill.
#
...
$env:SKILL_NAME = 'jira-tickets'
$env:SKILL_TAG_PREFIX = 'jira-v'
if (-not $env:SKILL_REPO) { $env:SKILL_REPO = 'diegoclair/skills' }
# Back-compat: forward the legacy per-skill version override if set.
if ($env:JIRA_TICKETS_VERSION -and -not $env:SKILL_VERSION) {
    $env:SKILL_VERSION = $env:JIRA_TICKETS_VERSION
}

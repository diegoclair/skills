# install.ps1 — PowerShell 5+ installer for jira-tickets (Windows)
#
# Usage (one-liner):
#   iwr -useb https://raw.githubusercontent.com/diegoclair/skills/main/jira-tickets/install/install.ps1 | iex
#
# Environment variables (all optional):
#   $env:JIRA_TICKETS_REPO      GitHub "owner/repo" (default: diegoclair/skills)
#   $env:CLAUDE_HOME          Override Claude home dir (default: $env:USERPROFILE\.claude)
#   $env:JIRA_TICKETS_VERSION   Specific release tag (default: latest)
...
$Repo        = if ($env:JIRA_TICKETS_REPO)    { $env:JIRA_TICKETS_REPO }    else { 'diegoclair/skills' }
$ClaudeHome  = if ($env:CLAUDE_HOME)         { $env:CLAUDE_HOME }         else { Join-Path $env:USERPROFILE '.claude' }
$SkillDir    = Join-Path $ClaudeHome 'skills\jira-tickets'
$BinDir      = Join-Path $SkillDir 'bin'
$BinName     = 'jira-tickets.exe'
...
Write-Host "Installing jira-tickets $Version for $Platform..."
...
$Archive     = "jira-tickets-$Platform.zip"
$DownloadUrl = "$GithubBase/releases/download/$Version/$Archive"
$TmpDir      = Join-Path $env:TEMP "jira-tickets-install-$(Get-Random)"
...
#   bin/jira-tickets.exe
#   SKILL.md
#   reference/*.md
$ExtractedBin = Join-Path $ExtractDir 'bin\jira-tickets.exe'
if (-not (Test-Path $ExtractedBin)) {
    # Fallback: recursive search in case the layout ever changes.
    $found = Get-ChildItem -Path $ExtractDir -Recurse -Filter 'jira-tickets*.exe' | Select-Object -First 1
...
# being executed — exactly what happens during `jira-tickets update`, which
...
Write-Host ""
Write-Host "Done. jira-tickets $Version installed to:"
...
if ($PathRegistered) {
    Write-Host ""
    Write-Host "Ready to use: ``jira-tickets --version`` from any new shell."
